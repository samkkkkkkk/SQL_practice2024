...
-- lower(소문자), initcap(앞 글자만 대문자), upper(대문자)
...
-- LTRIM(), RTRIM()
-- TRIM() -> 양쪽 공백 제거
-- LTRIM(param1, param2) -> param2의 값을 praram1에서 찾아서 제거 (왼쪽부터)
-- RTRIM(param1, param2) -> param2의 값을 param1에서 찾아서 제거 (오른쪽 부터)
SELECT 
 LTRIM('javascript', 'java')
FROM dual;
...
-- REPLACE(str, old, new)
SELECT
    REPLACE('My dreamis a president', 'president', 'programmer')
FROM dual;
...
SELECT
    REPLACE(CONCAT('(02)', SUBSTR(phone_number,5)), '.', '-')
FROM employees;
